...
import { useHighestAmountOfWalletRecharge } from "../hooks/useHighestAmountOfWalletRecharge";
import { IhighestAmountData, UserProfile } from "../interfaces";
import SearchModal from "./SearchModal";

const HighestAmountOfWalletRecharge = () => {
  const {
    searchData,
    setSearchData,
    searchBtnHandler,
    ...
    setSearch,
    searchHandler,
  } = useHighestAmountOfWalletRecharge();
      ...
      <Card className="px-4 py-6 mt-6 overflow-visible">
        <CardHeader className="flex flex-col lg:flex-row justify-between p-0 mb-6 text-asiatech-gray-800">
          <div className="flex gap-2">
            ...
            <span className="font-extrabold text-base">
              گزارش مشترکین با بیشترین مقدار شارژ کیف پول
            </span>
          </div>
        ...
        >
          <TableHeader>
            ...
            <TableColumn key="count"> شناسه کاربری</TableColumn>
          ...
          >
            {tableData &&
              tableData?.map((data: IhighestAmountData, index: number) => {
                return (
                  <TableRow className="bordertabel" key={index}>
                    ...
                    <TableCell>{NumberSeparator(data?.total_invoice_amount)} ریال </TableCell>
                  </TableRow>
                );
              })}
          </TableBody>
...
export default HighestAmountOfWalletRecharge;
